...
public class AccountManageServiceImpl implements AccountManageService,AccountManagementService,AccountManageDubboService,
		IAPIService {
...
//				}
		...
		if (user.referUserId != null && !"".equals(user.referUserId)) {
			...
			finanaceAccount.setReferUserId(finanaceEntry.getReferEntryId());
		}
...
//			}
	...
	public CommonResponse verifyAccount(User user) {
		...
		logger.info("检查账户有效性 入参:userId="+user.userId+",constId="+user.constId+",productId="+user.productId
		        +",referUserId="+user.referUserId);
		//校验数据合法性
...
//				}
    ...
    @Override
    @Transactional
    public String newBindingBankAccount(User user, AccountInfo accountInfo,ValidateData data) {
        logger.info("绑定银行卡  入参AccountInfo:" + BeanUtil.getBeanVal(accountInfo, null));
        logger.info("绑定银行卡  入参ValidateData:" + BeanUtil.getBeanVal(data, null));
        logger.info("--用户ID--:" + user.userId + "--机构号--：" + user.constId + "--产品号--：" + user.productId + "--用户名--" + user.userName + "第三方用户ID" + user.referUserId);
        synchronized (lock) {
            ...
            try {
                ...
                //根据 卡属性，及 finaccountid 查找名下的所有绑卡信息
                AccountInfoQuery accountInfoQuery = new AccountInfoQuery();
                //accountInfoQuery.setAccountProperty(accountInfo.getAccountProperty());
                accountInfoQuery.setFinAccountId(finanaceAccount.getFinAccountId());
                //finAccountId在所有机构内是唯一的，所以不需要再传入机构号
                List<AccountInfo> accountInfoList = accountInfoManager.queryList(accountInfoQuery);
                List<Integer> delAccountIdList = new ArrayList<Integer>();
                if(accountInfoList != null && !accountInfoList.isEmpty()){
                    StringBuffer sb = new StringBuffer();
                    for(AccountInfo selAccountInfo : accountInfoList){
                        int accountInfoStatus = selAccountInfo.getStatus();
                        //当卡号相同，且卡状态为 1/2/3/4时,证明卡已经绑定过
                        if(accountInfo.getAccountNumber().equals(selAccountInfo.getAccountNumber())
                                && isEquals(accountInfoStatus)){
                            logger.info("该卡号已经绑定过，请勿重复绑定！");
                            msg = "该卡号已经绑定过，请勿重复绑定！";
                            return msg;
                        }
                        if(Constants.ACCOUNT_PROPERTY_PRIVATE.equals(accountInfo.getAccountProperty())
                                && !accountInfo.getCertificateNumber().equals(selAccountInfo.getCertificateNumber())
                                && Constants.ACCOUNT_NUM_STATRS_1 == accountInfoStatus){
                            logger.info("绑定对私卡时证件号必须相同！");
                            msg = "绑定对私卡时证件号必须相同！";
                            return msg;
                        }
                        //当状态为失效且卡属性相同,卡号相同时，存入准备删除的集合中
                        if(accountInfo.getAccountProperty().equals(selAccountInfo.getAccountProperty()) 
                                && Constants.ACCOUNT_NUM_STATRS_0 == selAccountInfo.getStatus()
                                && accountInfo.getAccountNumber().equals(selAccountInfo.getAccountNumber())){
                            delAccountIdList.add(selAccountInfo.getAccountId());
                        }else if(Constants.ACCOUNT_NUM_STATRS_0 == selAccountInfo.getStatus()){
                            logger.info("存在无效卡 ：" + selAccountInfo.getAccountNumber());
                            continue;
                        }else{
                            sb.append(selAccountInfo.getAccountPurpose());
                            sb.append(",");
                        }
                    }
                    String accountPurpose = sb.toString();
                    String isOk = "ok";
                    if ((Constants.ACCOUNT_PURPOSE_1.equals(accountInfo.getAccountPurpose()) 
                            && accountPurpose.contains(accountInfo.getAccountPurpose()))
                                || (Constants.ACCOUNT_PURPOSE_1.equals(accountInfo.getAccountPurpose()) 
                                     && accountPurpose.contains(Constants.ACCOUNT_PURPOSE_4))) {
                        isOk = "结算卡已经存在，请绑定其它卡！";
                    }else if ((Constants.ACCOUNT_PURPOSE_3.equals(accountInfo.getAccountPurpose()) 
                            && accountPurpose.contains(accountInfo.getAccountPurpose())) 
                                || (Constants.ACCOUNT_PURPOSE_3.equals(accountInfo.getAccountPurpose()) 
                                    && accountPurpose.contains(Constants.ACCOUNT_PURPOSE_4))) {
                        isOk = "提现卡已经存在，请绑定其它卡！";
                    }else if ((Constants.ACCOUNT_PURPOSE_4.equals(accountInfo.getAccountPurpose()) 
                            && accountPurpose.contains(accountInfo.getAccountPurpose())) 
                                || (Constants.ACCOUNT_PURPOSE_4.equals(accountInfo.getAccountPurpose()) 
                                        && (accountPurpose.contains(Constants.ACCOUNT_PURPOSE_1) 
                                                || accountPurpose.contains(Constants.ACCOUNT_PURPOSE_3)))) {
                        isOk = "结算卡或者提现卡或者结算提现一体卡已经存在，请绑定其它卡！";
                    }
                    if(!"ok".equals(isOk)){
                        logger.info(isOk);
                        msg = isOk;
                ...
                if((Constants.ACCOUNT_PROPERTY_PRIVATE).equals(accountInfo.getAccountProperty())){
                    ...
                    msg = corporatAccountInfoService.accountInfoCheck(accountInfo,user.constId);
                    if("ok".equals(msg)){
                        accountInfo.setStatus(Constants.ACCOUNT_NUM_STATRS_1);
                ...
                if (data != null) {
                    //判断开户时的证件号是否相同    
                    String bkYZJG = parameterInfoService.getParameterValue(RedisConstants.PARAMETER_INFO_CODE_BK_YZJG);
                    logger.info("判断开户证件号是否相同的机构 ：" + bkYZJG);
                    if(bkYZJG != null && !"".equals(bkYZJG) && bkYZJG.contains(data.getMerchantId()) && accountInfo.getAccountProperty().equals(Constants.ACCOUNT_PROPERTY_PRIVATE)){
                        FinanacePersonQuery finanacePersonQuery = new FinanacePersonQuery();
                        finanacePersonQuery.setFinAccountId(accountInfo.getFinAccountId());
                        List<FinanacePerson> finPersons = finanacePersonDao.selectByExample(finanacePersonQuery);
                        if(finPersons != null && finPersons.size()>0){                                  
                ...
                //此处判断，需要是否存在需要删除的无效卡信息，有则删除
                if(delAccountIdList != null && !delAccountIdList.isEmpty()){
                    for(Integer delAccountId : delAccountIdList){
                        logger.info("无效卡信息已删除accountid = " + delAccountId);
                        accountInfoManager.deleteAccountInfoById(delAccountId);
                    }
                }
                accountInfoManager.saveAccountInfo(accountInfo);
            }catch (Exception e) {
                logger.error("绑定账户信息失败！" , e);
                msg = "绑定账户信息失败！";
            }
            return msg;
        }
    }
    private boolean isEquals(int accountInfoStatus){
        boolean flag = false;
        if(Constants.ACCOUNT_NUM_STATRS_1 == accountInfoStatus
                || Constants.ACCOUNT_NUM_STATRS_2 == accountInfoStatus
                || Constants.ACCOUNT_NUM_STATRS_3 == accountInfoStatus
                || Constants.ACCOUNT_NUM_STATRS_4 == accountInfoStatus){
            flag = true;
        }
        return flag;
    }
...
//						data.setUserName(value);
			...
			String msg = this.newBindingBankAccount(user, accountInfo,data);
			if ("ok".equals(msg)) {
				response.setIs_success(true);
			} else {
