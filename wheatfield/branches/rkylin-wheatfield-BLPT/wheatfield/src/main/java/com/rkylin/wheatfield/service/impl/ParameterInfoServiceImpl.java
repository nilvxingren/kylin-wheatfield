...
public class ParameterInfoServiceImpl implements ParameterInfoService {
  ...
  public String getParameterValue(String parameterCode) {
      ...
      try{
          ...
          for(ParameterInfo pi : parameterInfos){
              String value = pi.getParameterValue();
          ...
          String resultValue = values.toString();
          returnValue = resultValue.substring(0, resultValue.length() - 1);
          redisTemplateMap.opsForValue().set(parameterCode,returnValue);
      }catch(Exception ex){
          logger.error("获取参数值异常：" , ex);
          returnValue = null;
      }
      return returnValue; 
  }
  ...
  @Override
  public Set<String> getParaValSetByParamCode(String parameterCode) {
      Set<String> instSet = redisTemplateSet.opsForValue().get(parameterCode);
      if (instSet!=null) {
          return instSet;
      }
      List<ParameterInfo> list = parameterInfoManager.getParameterInfoByParamCode(parameterCode);
      ...
      for (ParameterInfo parameterInfo : list) {
          instSet.add(parameterInfo.getParameterValue());
      }
      redisTemplateSet.opsForValue().set(parameterCode, instSet);
      return instSet; 
   } 
 }
