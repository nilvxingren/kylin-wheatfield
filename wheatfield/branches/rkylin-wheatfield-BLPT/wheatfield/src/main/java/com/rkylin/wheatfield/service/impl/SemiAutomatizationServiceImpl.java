...
public class SemiAutomatizationServiceImpl implements SemiAutomatizationService,SemiAutomatizationServiceApi{
    ...
    public CommonResponse operateFinAccounts(List<com.rkylin.wheatfield.bean.User> userList){
        logger.info("账户资金操作  传入参数:" + BeanUtil.getBeanListVal(userList, null));
        CommonResponse res = new CommonResponse();
        res.setCode(CodeEnum.FAILURE.getCode());
        if(userList == null||userList.size()==0){
            res.setMsg("参数为空!");
            logger.info("账户资金操作 参数为空!");
            return res;
        }
        Set<String> userInfoSet = new HashSet<String>();
        for (com.rkylin.wheatfield.bean.User user : userList) {
            String field = BeanUtil.validateBeanProEmpty(user,new String[] { "userId", "instCode", "productId","amount","status" });
            if (field != null) {
                res.setMsg(field + "不能为空");
                logger.info("账户资金操作"   +field + "不能为空");
                return res;
            }
            ...
            String userInfo = user.getUserId()+user.getInstCode()+user.getProductId();
            if (userInfoSet.contains(userInfo)) {
                res.setMsg("同一个账户不能操作两次,请合并参数!");
                logger.info("参数对同一账户操作两次!");
                return res;
            }
        ...
        for (com.rkylin.wheatfield.bean.User user : userList) {
            ...
            paramMap.put("type", "1001");
            paramMap.put("amount", String.valueOf(user.getAmount()));
            paramMap.put("userid", user.getUserId());
            paramMap.put("consid", user.getInstCode());
            paramMap.put("productid", user.getProductId());
            paramMap.put("status", String.valueOf(user.getStatus()));
            FinanaceEntry finanaceEntry = dbOperate(paramMap,null);
            if(finanaceEntry == null){
                res.setMsg("操作失败!");
                logger.info("账户资金操作  生成流水失败!");
                return res;
            }
            finanaceEntry.setReferId(user.getFinAccountId());
            finanaceEntry.setRemark("账户资金操作"); 
            finanaceEntryList.add(finanaceEntry); 
        }
        finanaceEntryManager.saveFinanaceEntryList(finanaceEntryList);
    ...
    public CommonResponse operateFinAccount(com.rkylin.wheatfield.bean.User user) {
        logger.info("账户资金操作  传入参数:" + BeanUtil.getBeanVal(user, null));
        CommonResponse res = new CommonResponse();
        res.setCode(CodeEnum.FAILURE.getCode());
        if(user == null){
            res.setMsg("参数为空!");
        ...
        String field = BeanUtil.validateBeanProEmpty(user,new String[] { "userId", "instCode", "productId","amount","status" });
        logger.info("账户资金操作 传入的参数字段校验 是否有空值(field的值为是空值的字段,为null表示都不为空)  field=" + field);
        if (field != null) {
            res.setMsg(field + "不能为空");
            logger.info("账户资金操作"   +field + "不能为空");
            return res;
