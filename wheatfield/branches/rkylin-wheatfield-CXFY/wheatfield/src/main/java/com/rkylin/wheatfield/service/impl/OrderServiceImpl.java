...
import com.rkylin.wheatfield.api.OrderServiceApi;
...
public class OrderServiceImpl implements OrderService,IAPIService,OrderDubboService,OrderServiceApi {
	...
	public Response doJob(Map<String, String[]> paramMap, String methodName) {
		...
		if("ruixue.wheatfield.ordernum.query".equals(methodName)){//获取账户订单条数
			...
			List<TransOrderInfoNew> transOrders=this.getTransOrdersNew(userId, orderNo, merchantCode, startTime,
					endTime, funcCode, status,interMerchantCode,amount,orderPackageNo);
	...
	@Override
	public List<TransOrderInfoNew> getTransOrdersNew(String userId,String orderNo,String merchantCode, String startTime, String endTime,String funcCode, String status,String interMerchantCode,String amount,String orderPackageNo) {
		logger.info("###################交易订单查询开始###############################");
		logger.info("交易查询入参相关信息：userId="+userId+";orderNo="+orderNo+";merchantCode="+merchantCode+
		        ",startTime="+startTime+";endTime="+endTime+";funcCode="+funcCode+";status="+status
		        +",interMerchantCode="+interMerchantCode+",amount="+amount+",orderPackageNo="+orderPackageNo);
		DateUtil dateUtil=new DateUtil();
		try{
			TransOrderInfoQuery query=new TransOrderInfoQuery();
			query.setOrderNo(orderNo);
			...
			query.setOrderPackageNo(orderPackageNo);
	...
	/**
	 * Discription: 查询订单信息，对外提供dubbo接口
	 * @return TransOrderResponse
	 * @author liuhuan
	 * @since 2016年12月9日
	 */
	...
	public TransOrderResponse getTransOrderInfo(TransOrderInfoPage transOrderInfoPage){
        logger.info("--------------dubbo调用交易订单查询开始---------------");
        TransOrderResponse transOrderResponse = new TransOrderResponse();
        if(transOrderInfoPage == null){
            transOrderResponse.setSuccess(false);
            transOrderResponse.setMsg("不可传入空POJO");
            logger.info("不可传入空POJO");
            return transOrderResponse;
        }
        logger.info("交易查询入参相关信息：userId = " + transOrderInfoPage.getUserId() + " ; orderNo = " + transOrderInfoPage.getOrderNo() + " ; merchantCode = " + transOrderInfoPage.getMerchantCode() + 
                " , startTime = " + transOrderInfoPage.getStartTime() + " ; endTime = " + transOrderInfoPage.getEndTime() + " ; funcCode = " + transOrderInfoPage.getFuncCode() + " ; status = " + transOrderInfoPage.getStatus()
                + " , interMerchantCode = " + transOrderInfoPage.getInterMerchantCode() + " , amount = " + transOrderInfoPage.getAmount() + " , orderPackageNo = " + transOrderInfoPage.getOrderPackageNo()
                + " ; pageNum = " + transOrderInfoPage.getPageNum() + " ; pageSize = " + transOrderInfoPage.getPageSize());
        
        String merchantCode = transOrderInfoPage.getMerchantCode();
        if(merchantCode == null || "".equals(merchantCode)){
            transOrderResponse.setSuccess(false);
            transOrderResponse.setMsg("merchantCode不能为空");
            logger.info("merchantCode不能为空");
            return transOrderResponse;
        }
        if((transOrderInfoPage.getStartTime() == null && transOrderInfoPage.getEndTime() != null)
                || (transOrderInfoPage.getStartTime() != null && transOrderInfoPage.getEndTime() == null)){
            transOrderResponse.setSuccess(false);
            transOrderResponse.setMsg("如按时间段查询,起始、结束时间均必填");
            logger.info("如按时间段查询,起始、结束时间均必填");
            return transOrderResponse;
        }
        try{
            Integer pageSize = transOrderInfoPage.getPageSize();
            transOrderInfoPage.setPageSize(pageSize == null || pageSize == 0 ? 100 : pageSize);
            if(transOrderInfoPage.getPageSize() > 1000){
                transOrderResponse.setSuccess(false);
                transOrderResponse.setMsg("每页显示条数不得大于1000");
                logger.info("每页显示条数不得大于1000");
                return transOrderResponse;
            }
            Integer pageNum = transOrderInfoPage.getPageNum();
            transOrderInfoPage.setPageNum(pageNum == null || pageNum == 0 ? 1 : pageNum);
            transOrderInfoPage.setStartRow(transOrderInfoPage.getPageSize() * (transOrderInfoPage.getPageNum() - 1));
            logger.info("startRow : " + transOrderInfoPage.getStartRow());
            List<TransOrderInfo> transOrderList = transOrderInfoDao.selTransOrderInfo(transOrderInfoPage);
            if(transOrderList == null || transOrderList.isEmpty()){
                transOrderResponse.setSuccess(false);
                transOrderResponse.setMsg("未查找到订单信息");
                logger.info("未查找到订单信息");
                return transOrderResponse;
            }
            int totalCount = transOrderInfoDao.selTransOrderInfoCount(transOrderInfoPage);
            List<TransOrderInfoNew> transOrders = new ArrayList<TransOrderInfoNew>();
            for (TransOrderInfo selTransOrderInfo: transOrderList) {
                TransOrderInfoNew order = new TransOrderInfoNew();
                order.setOrderNo(selTransOrderInfo.getOrderNo());
                order.setAmount(selTransOrderInfo.getAmount());
                order.setOrderStatus(selTransOrderInfo.getStatus());               
                order.setFuncCode(selTransOrderInfo.getFuncCode());
                order.setOrderPackageNo(selTransOrderInfo.getOrderPackageNo());
                order.setRequestNo(selTransOrderInfo.getRequestNo());
                order.setCreatedTime(selTransOrderInfo.getCreatedTime());
                order.setUpdatedTime(selTransOrderInfo.getUpdatedTime());
                order.setTranssumid(selTransOrderInfo.getTranssumid());
                order.setUserId(selTransOrderInfo.getUserId());
                order.setInterMerchantCode(selTransOrderInfo.getInterMerchantCode());
                order.setErrorMsg(null == selTransOrderInfo.getErrorCode() ? "" : selTransOrderInfo.getErrorCode());
                order.setMerchantCode(selTransOrderInfo.getMerchantCode());
                transOrders.add(order);
            }
            transOrderResponse.setSuccess(true);
            transOrderResponse.setTransOrderInfoNews(transOrders);
            transOrderResponse.setTotalCount(totalCount);
            transOrderResponse.setMsg("success");
            logger.info("-------------交易订单查询结束------------");
        }catch(Exception ex){
            logger.error("查询交易信息异常：" , ex);
            transOrderResponse.setSuccess(false);
            transOrderResponse.setMsg("查询交易信息异常");
        }
        logger.info("--------------dubbo调用交易订单查询结束---------------");
        return transOrderResponse;
	}
}
