...
//				}
     ...
     * @since 2016年11月18日
     */
    public void submitToCollSysPayJDY1() {
        logger.info("---------"+Constants.JDY_ID+"---代付--扫描数据库提交到代收付系统任务开始--------------------------------------");
        synchronized (lock) {
            String accountDate=generationPaymentService.getAccountDate();
            try {
                generationPaymentService.submitToRecAndPaySys(BaseConstants.FILE_TYPE_7, Constants.JDY_ID,
                        accountDate,DateUtils.getAccountDate(Constants.DATE_FORMAT_YYYYMMDD, accountDate));
            } catch (Exception e) {
                logger.error(e.getMessage(),e);
                logger.info("--------------------------扫描数据库提交到代收付系统任务异常--------------------------------------");
        ...
        logger.info("---------"+Constants.JDY_ID+"---代付--扫描数据库提交到代收付系统任务结束--------------------------------------");
    }   
}
