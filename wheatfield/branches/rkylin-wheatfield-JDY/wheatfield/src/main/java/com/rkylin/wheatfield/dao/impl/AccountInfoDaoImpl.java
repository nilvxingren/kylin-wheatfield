...
public class AccountInfoDaoImpl extends BaseDao implements AccountInfoDao {
	...
	public int deleteByPrimaryKey(Integer accountId) {
		return super.getSqlSession().delete("AccountInfoMapper.deleteByPrimaryKey", accountId);
	}
	...
	*/
    ...
    public List<AccountInfo> selectAccInfo(AccountInfoQuery example) {
	    List<AccountInfo> list = super.getSqlSession().selectList("AccountInfoMapper.selectAccInfoPerson", example);
	    if (list.size()==0) {
	        list = super.getSqlSession().selectList("AccountInfoMapper.selectAccInfoCompany", example);
        }
        return list;
    }
}
