...
//				creditApprovalInfo.setCreditAgreementId(fileMap.get("L_15"));//授信协议ID
	...
	public CommonResponse paymentGeneration(String generationType,String merchantId,int orderType,String bussinessCode,int dateType) {
        ...
        //定义新类型，当为新汇总方法时，新类型会被赋值
        String newType = "";
        if(TransCodeConst.SUMMARY_ALL.equals(generationType)){
        ...
        if(TransCodeConst.SUMMARY_ALL.equals(newType)){
            query.setAccountDate(accountDateLast);
            query.setBusiTypeId(newType);
        ...
        if(dateType==0&&!TransCodeConst.PAYMENT_40143.equals(generationType)&&!TransCodeConst.SUMMARY_ALL.equals(newType)&&!TransCodeConst.PAYMENT_40144.equals(generationType)){
            ...
            //2016-11-21 18:35:13 逻辑调账  RZL
//            else if(!Constants.KZ_ID.equals(merchantId)&&!Constants.BBZX_ID.equals(merchantId)&&!Constants.YSJ_ID.equals(merchantId)
//                    &&!Constants.LY_ID.equals(merchantId)&&!Constants.TLZX_ID.equals(merchantId)&&!Constants.FN_ID.equals(merchantId)
//                    &&!Constants.MRFQ_ID.equals(merchantId)){
//                query.setErrorMsg(TransCodeConst.T0_FLAG);
//            }
            //  //2016-11-21 18:35:13 逻辑调账  RZL
            else if(Constants.HT_ID.equals(merchantId)){
            	query.setErrorMsg(TransCodeConst.T0_FLAG);
            }
        }
...
//                }
            ...
            if(TransCodeConst.SUMMARY_ALL.equals(newType)){
                //当标志为全量汇总时，备注赋值为busitypeid
                generationPayment.setRemark(transOrderInfo.getBusiTypeId());
            }else{
...
//						interestRepaymentManager.saveInterestRepayment(interestRepayment);
						...
						if (Constants.HT_ID.equals(rootInstId)) {
						    ...
						    productId = Constants.LY_PRODUCT;
						}else if(Constants.TLZX_ID.equals(rootInstId)){
                            productId = Constants.TLZX_PRODUCT;
                        }else if(Constants.MRFQ_ID.equals(rootInstId)){
                            productId = Constants.MRFQ_PRODUCT;
                        }else if(Constants.XR_ID.equals(rootInstId)){
                            productId = Constants.XR_PRODUCT;
                        }else{
							flag = false;
						}
						if(flag){
							...
							finanaceEntry.setPaymentAmount(generationPayment.getAmount());
							finanaceEntry.setReferId(generationPayment.getOrderNo());
							if (!TransCodeConst.NO_TALLY.equals(generationPayment.getRemark())) {
