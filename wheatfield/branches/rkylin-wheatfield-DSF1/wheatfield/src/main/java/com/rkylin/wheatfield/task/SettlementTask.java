...
import com.rkylin.wheatfield.settlement.LianlianLogic;
...
public class SettlementTask extends BaseDao {
	...
	public void collate_TLHT_C() {
		...
		Map<String,String> rtnMap = settlementService.readCollateFileHT(null,"'"+Constants.HT_ID+"'","26","","B1");
...
//		logger.info("生成对账文件的返回值："+rtnStr);
	...
	//连连支付对账
	public void collate_LL_C() {
		logger.info("连连支付对账计划任务-----------START----------------"+new Date());
//		Map<String,String> rtnMap = lianlianLogic.readCollateFileLL(null,null,null);
//		if (rtnMap !=null && "0000".equals(rtnMap.get("errCode"))) {
//			logger.info("连连支付对账计划任务成功！");
//		}else {
//			if (rtnMap !=null) {
//				RkylinMailUtil.sendMailThread("连连支付对账计划任务","连连支付对账计划任务:["+rtnMap.get("errCode")+"]-["+rtnMap.get("errMsg")+"]", "21401233@qq.com");
//				logger.error("连连支付对账计划任务:["+rtnMap.get("errCode")+"]-["+rtnMap.get("errMsg")+"]");
//			} else {
//				RkylinMailUtil.sendMailThread("连连支付对账计划任务","连连支付对账计划任务失败！", "21401233@qq.com");
//				logger.error("连连支付对账计划任务失败！");
//			}
//		}
		logger.info("连连支付对账计划任务-----------END----------------"+new Date());
	}
	
	//连连支付&君融贷对账
	public void collate_M5LL_C() {
		logger.info("连连支付&君融贷对账计划任务-----------START----------------"+new Date());
		Map<String,String> rtnMap = lianlianLogic.readCollateFileM5LL(null,"'M000005'",null);
		if (rtnMap !=null && "0000".equals(rtnMap.get("errCode"))) {
			logger.info("连连支付&君融贷对账计划任务成功！");
		}else {
			if (rtnMap !=null) {
				RkylinMailUtil.sendMailThread("连连支付&君融贷对账计划任务","连连支付&君融贷对账计划任务:["+rtnMap.get("errCode")+"]-["+rtnMap.get("errMsg")+"]", "21401233@qq.com");
				logger.error("连连支付&君融贷对账计划任务:["+rtnMap.get("errCode")+"]-["+rtnMap.get("errMsg")+"]");
			} else {
				RkylinMailUtil.sendMailThread("连连支付对账计划任务","连连支付&君融贷对账计划任务失败！", "21401233@qq.com");
				logger.error("连连支付&君融贷对账计划任务失败！");
			}
		}
		logger.info("连连支付&君融贷对账计划任务-----------END----------------"+new Date());
	}
