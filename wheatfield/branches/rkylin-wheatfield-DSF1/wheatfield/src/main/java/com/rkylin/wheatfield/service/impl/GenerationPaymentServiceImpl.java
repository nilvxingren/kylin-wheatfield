...
//			String oneKey = CommUtil.getGenerateId();
			...
			synchronized (lock) {
				...
				redisBase.saveOrUpdateStrSet(RedisConstants.GENERATIONPAYMENT_PUSH, oneKey, 4, TimeUnit.HOURS, false);
				logger.info("存储之后  所有缓存的  数据的   key ===" + redisBase.getSet(RedisConstants.GENERATIONPAYMENT_PUSH));
				redisBase.saveOrUpdateList(oneKey, orderDetailList, 50, TimeUnit.MINUTES);
			} 
		}
...
//		Long.parseLong("shdh");
	 ...
	 * @return
	 */
	public CommonResponse getResultsFromRecAndPaySys(String batch,String[] orderNoArray){
