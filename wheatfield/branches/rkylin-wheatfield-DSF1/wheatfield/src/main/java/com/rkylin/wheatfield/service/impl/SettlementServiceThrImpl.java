...
public class SettlementServiceThrImpl extends BaseDao implements
		SettlementServiceThr, IAPIService {
	...
	public Map<String,String> getCreditInfo(String type,String accountDate,String batch) {
		...
		try {
			...
			if (!"0000".equals(tmpMap.get("errCode"))) {
				rtnMap = tmpMap;
				logger.error("文件下载！"+rtnMap.get(rtnMap.get("errMsg")));
				return rtnMap;
			}
		} catch (Exception e) {
...
//					e.printStackTrace();
			...
			if(Constants.KZ_ID.equals(transDaysSummary.getRootInstCd())||Constants.ZK_ID.equals(transDaysSummary.getRootInstCd())){
