...
public class InterestRepaymentHTServiceImpl implements InterestRepaymentHTService {
	...
	public boolean  repayForCapitalDetail(InterestRepayment repayment){
		...
		TransferInfo tsLoan = null;
		if(amountLoan > 0){
			CreditApprovalInfo creditApprovalInfo = this.getCreditApprovalInfo(repayment);
			if(creditApprovalInfo == null){
				logger.info("=======================InterId:"+repayment.getInterId()+"  错误原因：没有找到授信信息，请及时处理==============================");
				this.sendMail("没有找到授信信息，请及时处理", "InterId:"+repayment.getInterId()+"  错误原因：没有找到授信信息，请及时处理");
			}
			long amountCr = this.querycreditAmount(repayment.getRootInstCd(), repayment.getUserId(), repayment.getProductId());
			if(amountCr < amountLoan){
				logger.info("=====================================InterId:"+repayment.getInterId()+"  错误原因：授信子账户的金额小于贷款剩余金额，请及时处理=======================================");
				this.sendMail("授信子账户的金额小于贷款剩余金额，请及时处理", "InterId:"+repayment.getInterId()+"  错误原因：授信子账户的金额小于贷款剩余金额，请及时处理");
			    return false;
			}
			tsLoan = transferFromCreditAccount(repayment, amountLoan, creditApprovalInfo);
		}
...
//		}
		// 本金还款
		TransferInfo tsCt = this.transferForCapital(repayment, amount - amountLoan);
	    if(tsCt.isSuccess()){
	    	// 修改还款状态为成功
			return true;
	    }else{
	    	//设置还款状态为失败
	    	if(tsLoan != null && tsLoan.isSuccess() && amountLoan > 0){
	...
	public TransferInfo transferForCapital(InterestRepayment repayment, long amount) {
		...
		if (null == itemId) {
			logger.info("==================InterId:"+repayment.getInterId()+" 获取到的orderitemid 为空，设置还款状态为失败=======================");
			return transferInfo;
		}
		// 应还金额转账
		...
		try{
			...
			this.sendMail("会唐本金还款，调用订单转账接口发生异常，请及时手工处理", "会唐本金还款，调用订单转账接口发生异常，请及时手工处理   InterId:"+repayment.getInterId()+"  错误原因："+e.getMessage());
		}
		if (restrans != null && "true".equals(restrans.getIs_success())) {
			...
			try{
				...
				this.sendMail("本金还款成功，但是更新订单状态发生异常，请及时手工处理", "本金还款成功，但是更新订单状态发生异常，请及时手工处理   InterId:"+repayment.getInterId()+"  错误原因："+e.getMessage());
			}
			transferInfo.setSuccess(true);
		} else {
			...
			try{
				...
				this.sendMail("本金还款失败，但是更新订单状态发生异常，请及时手工处理", "本金还款失败，但是更新订单状态发生异常，请及时手工处理   InterId:"+repayment.getInterId()+"  错误原因："+e.getMessage());
			}
		}
		return transferInfo;
	}
	 ...
	 * 贷款余额转账，从授信子账户
	 */
	public TransferInfo transferFromCreditAccount(InterestRepayment repayment, long amountLoan, CreditApprovalInfo creditApprovalInfo) {
		TransferInfo transferInfo = new TransferInfo();
//		if(amountLoan <= 0){
//			transferInfo.setSuccess(true);
//        	return transferInfo;
...
//		CreditApprovalInfo creditApprovalInfo = list.get(0);
		...
		try{
			...
			logger.info(e.getMessage());
			this.sendMail("会唐使用贷款余额还款，调用订单转账接口发生异常", "会唐使用贷款余额还款，调用订单转账接口发生异常 请及时手工处理   InterId:"+repayment.getInterId()+"  错误原因："+e.getMessage());
		}
		if (restrans != null && "true".equals(restrans.getIs_success())) {
			logger.info("==================InterId:"+repayment.getInterId()+" 转账成功==============================");
			try{
			...
			logger.info("==================InterId:"+repayment.getInterId()+" 转账失败，设置订单状态为失败=======================");
			try{
				this.updateItemFail(itemId);
			}catch(Exception e){
				logger.info("==================InterId:"+repayment.getInterId()+" 本金还款失败，但是更新订单状态失败  "+e.getMessage()+" =======================");
	...
	public long querycreditAmount(String rootInstCd, String userId, String productId) {
		FinanaceAccountQuery faQuery = new FinanaceAccountQuery();
		faQuery.setRootInstCd(rootInstCd);
		faQuery.setAccountRelateId(userId);
		faQuery.setFinAccountTypeId("10002");
		faQuery.setGroupManage(productId);
		List<FinanaceAccount> list = finanaceAccountManager.queryList(faQuery);
		if (list == null || list.size() == 0) {
			// 没有找到主账户
			logger.info("=====================没有授信子账户=======================");
			throw new RuntimeException("没有授信子账户");
		}
		if (list.size() > 1) {
			// 主账户不唯一
			logger.info("========================授信子账户不唯一=========================");
			throw new RuntimeException("授信子账户不唯一");
		}
		long amount = list.get(0).getAmount();
		return amount;
	}
	...
	public CreditApprovalInfo getCreditApprovalInfo(InterestRepayment repayment){
		 CreditApprovalInfoQuery crquery = new CreditApprovalInfoQuery();
	        crquery.setRootInstCd(repayment.getRootInstCd());
	        crquery.setProviderId(repayment.getProviderId());
	        crquery.setUserId(repayment.getUserId());
