...
public class FinanacePersonDaoImpl extends BaseDao implements FinanacePersonDao {
    ...
    @Override
    public List<FinanacePerson> selectFinPersons(FinanacePersonQuery query) {
        if (query.getDataIndex() == null) {
            query.setDataIndex(0);
        }
        return super.getSqlSession().selectList("FinanacePersonMapper.selectFinPersons", query);
    }
}
