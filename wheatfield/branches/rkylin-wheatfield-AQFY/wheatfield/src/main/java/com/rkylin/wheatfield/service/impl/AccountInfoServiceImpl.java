...
import com.rkylin.wheatfield.bean.FinPersonQuery;
...
import com.rkylin.wheatfield.response.FinanaceCompanyResponse;
import com.rkylin.wheatfield.response.FinanacePersonResponse;
...
//            }
    ...
    @Override
    public FinanaceCompanyResponse getFinCompanyInfo(com.rkylin.wheatfield.bean.FinCompanyQuery query) {
        logger.info("获取企业开户信息:"+BeanUtil.getBeanVal(query, null));
        FinanaceCompanyResponse res = new FinanaceCompanyResponse();
        res.setCode(CodeEnum.FAILURE.getCode());
        if (query==null) {
            logger.info("参数为空");
            res.setMsg("参数为空!"); 
        ...
        if (query.getRootInstCd()==null || "".equals(query.getRootInstCd())) {
            logger.info("机构号为空");
            res.setMsg("机构号为空!"); 
            return res;
        }
        com.rkylin.wheatfield.pojo.FinanaceCompanyQuery finCompanyQuery = new FinanaceCompanyQuery();
        BeanMapper.copy(query, finCompanyQuery);
        if (query.getDataSize()==null) {
            String queryCountLimit = parameterInfoService.getParameterValByParameterCode(RedisConstants.QUERY_COUNT_LIMIT_KEY);
            if (queryCountLimit==null) {
                logger.info("获取数据库或缓存的数据上限值异常,key="+RedisConstants.QUERY_COUNT_LIMIT_KEY);
                res.setMsg("系统异常!"); 
                return res; 
            }
            finCompanyQuery.setDataSize(Integer.parseInt(queryCountLimit));
        }
        List<FinanaceCompany> finanaceCompanyList = finanaceCompanyDao.selectFinCompanys(finCompanyQuery);
        logger.info("获取企业开户信息查出的个数:"+finanaceCompanyList.size());
        if (finanaceCompanyList.size()==0) {
            res.setMsg("没有查出数据!"); 
            return res;
        }
        res.setFinanaceCompanyList(finanaceCompanyList);
        res.setCode(CodeEnum.SUCCESS.getCode());
        return res;
    }
    ...
    public FinanacePersonResponse getFinPersonInfo(FinPersonQuery query) {
        logger.info("获取个人开户信息:"+BeanUtil.getBeanVal(query, null));
        FinanacePersonResponse res = new FinanacePersonResponse();
        res.setCode(CodeEnum.FAILURE.getCode());
        if (query==null) {
            logger.info("参数为空");
            res.setMsg("参数为空!"); 
        ...
        com.rkylin.wheatfield.pojo.FinanacePersonQuery finPersonQuery = new FinanacePersonQuery();
        BeanMapper.copy(query, finPersonQuery);
        if (query.getDataSize()==null) {
            String queryCountLimit = parameterInfoService.getParameterValByParameterCode(RedisConstants.QUERY_COUNT_LIMIT_KEY);
            if (queryCountLimit==null) {
                logger.info("获取数据库或缓存的数据上限值异常,key="+RedisConstants.QUERY_COUNT_LIMIT_KEY);
            ...
            finPersonQuery.setDataSize(Integer.parseInt(queryCountLimit));
        }
        List<FinanacePerson> finanacePersonList = finanacePersonDao.selectFinPersons(finPersonQuery);
        logger.info("获取个人开户信息查出的个数:"+finanacePersonList.size());
        if (finanacePersonList.size()==0) {
            res.setMsg("没有查出数据!"); 
            return res;
        }
        res.setFinanacePersonList(finanacePersonList);
        res.setCode(CodeEnum.SUCCESS.getCode());
        return res;
    }
