...
public class FinanaceCompanyDaoImpl extends BaseDao implements FinanaceCompanyDao {
    ...
    public List<FinanaceCompany> selectByRootInstCdOrBUSLINCEOrStatusId(FinanaceCompanyQuery example) {
        return super.getSqlSession().selectList("FinanaceCompanyMapper.selectByRootInstCdOrBUSLINCEOrStatusId",
                example);
    }
    ...
    @Override
    public List<FinanaceCompany> selectFinCompanys(FinanaceCompanyQuery query) {
        if (query.getDataIndex() == null) {
            query.setDataIndex(0);
        }
        return super.getSqlSession().selectList("FinanaceCompanyMapper.selectFinCompanys", query);
    }
