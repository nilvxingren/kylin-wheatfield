...
public class GenerationPaymentHistoryDaoImpl extends BaseDao implements GenerationPaymentHistoryDao {
    ...
    @Override
    public List<GenerationPaymentHistory> selectByOrderNoAndInstcode(GenerationPaymentHistoryQuery query) {
        return super.getSqlSession().selectList("GenerationPaymentHistoryMapper.selectByOrderNoAndInstcode", query);
    }
    
    @Override
    public int batchUpdate( List<?> list){
        return super.batchUpdate("GenerationPaymentHistoryMapper.updateByPrimaryKeySelective", list);
    }
}
