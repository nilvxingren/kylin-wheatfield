...
public class RedisTest extends BaseJUnit4Test {
   ...
   @Test
    public void getIncrementLongTest() {
        for (int i = 0; i < 1; i++) {
            Set<String> orderNoSet = new HashSet<String>();
            orderNoSet.add("wwwwwwwwwwwwww");
//            Long no = redisBase.getIncreLong("PINGAN_SUB_ACCOUNT_SEQ",20010911l);
//            logger.info("================no  ====  "+no);
...
//			redisBase.delSet(key);	
    ...
    public void getSetTest2() {
        String key = "HFYRHFYRHFRYF";
        redisBase.saveOrUpdateStrSet(RedisConstants.RECEIVE_AND_PAYMENT_PUSH_ALL_KEYS, key, 4, TimeUnit.HOURS, false);
        List<OrderDetail> orderDetailList = new ArrayList();
        OrderDetail orderDe = new OrderDetail();
        orderDe.setOrderNo("1451359098219182554");
        orderDe.setStatusId(15);
