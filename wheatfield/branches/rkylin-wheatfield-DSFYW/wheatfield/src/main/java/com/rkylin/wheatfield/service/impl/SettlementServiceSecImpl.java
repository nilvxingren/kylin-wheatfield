		...
		try{
			for (Object keyObj : paramMap.keySet().toArray()) {
				...
				for (String value : strs) {
					...
					}else if(keyObj.equals("advanceDate")){
						rateTemp.setAdvanceDate(value);
					}
				}
			}
		}catch (Exception ex){
...
//paraMap.put("status", "4");
	...
	private CreditRateTemplate setRateTemp(String type ,CreditRateTemplateRes rateTemp){
		...
		rateInsert.setAdvanceDate(rateTemp.getAdvanceDate());
		return rateInsert;
	}
	...
	* @return
	 */
	@Override
	public SettlementSecResponse queryFinanaceentryList(String rootinstcd, String userid,
...
//			HashMap<String,String> rtnMap = new HashMap<String,String>();
			...
			if("1".equals(querytype)){//根据账户查询 
				if (null == rootinstcd || "".equals(rootinstcd)) {
					response.setIs_success(false);
					response.setRtn_msg("机构号不能为空");
					return response;
				}	
				if (null == userid || "".equals(userid)) {
					response.setIs_success(false);
					response.setRtn_msg("用户id不能为空");
					return response;
				}
				if ((null != createdtimeto && !"".equals(createdtimeto)) && (null == createdtimefrom || "".equals(createdtimefrom))) {
					response.setIs_success(false);
					response.setRtn_msg("记录创建结束时间不为空时，记录创建开始时间不能为空");
					return response;
				}
				...
				if (null == requestid || "".equals(requestid)) {
					response.setIs_success(false);
					response.setRtn_msg("交易记录编码不能为空");
					return response;
				}
