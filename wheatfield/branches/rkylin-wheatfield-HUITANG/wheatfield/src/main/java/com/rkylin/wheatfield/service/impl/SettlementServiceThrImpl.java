...
public class SettlementServiceThrImpl extends BaseDao implements
	...
	public CommonResponse notifyCreditReslts(String fileTpye,String accountDate,String batch,String type){
		...
		if ("1".equals(type)) {// 审核授信接口
			...
			logger.info("审核授信接口=======code==="+res.getCode()+",msg="+res.getMsg());
			return res;
		}  
		if("3".equals(type)){
			//通知分润
		...
		logger.info("通知分润=======code==="+res.getCode()+",msg="+res.getMsg());
		return res;
	}
...
//				creditApprovalInfo.setCreditAgreementId(fileMap.get("L_15"));//授信协议ID
	...
	public void paymentGeneration(String generationType,String merchantId,int orderType,String bussinessCode,int dateType) {
		...
		try {
			//获取当前账期
			Date accountDateLast=dateUtil.parse(operationServive.getAccountDate(), "yyyy-MM-dd");//设置账期T-1
			//获取T-1日账期
			...
			if(Constants.HT_ID.equals(merchantId) && TransCodeConst.PAYMENT_COLLECTION.equals(generationType)){
				Date date = dateUtil.parse(dateUtil.getDate()+" 16:00:00", "yyyy-MM-dd hh:mm:ss");//获取当天的下午四点时间
				cal.setTime(date);
			...
			List<Map<String,Object>> upTransList = new ArrayList();
			if(null!=transOrderInfos&&0<transOrderInfos.size()){
				...
				for (TransOrderInfo transOrderInfo : transOrderInfos) {
					...
					logger.info( merchantId + "用户"+ transDaysSummary.getUserId() + "共" + generationType +"金额:"+transOrderInfo.getAmount());					
				}
			}		
			...
			if(null!=transDaysSummaries&&0<transDaysSummaries.size()){
				for (TransDaysSummary transDaysSummary : transDaysSummaries) {
					...
					for (TransOrderInfo transOrderInfo:transOrderInfos) {
						if (transOrderInfo.getOrderNo().equals(transDaysSummary.getSummaryOrders()) && "1".equals(transOrderInfo.getBusiTypeId())) {
					...
					//特殊初理：丰年提现的场合，特殊的卡类型校验
//					if(Constants.FN_ID.equals(merchantId) && TransCodeConst.WITHDROW.equals(generationType)){
//						accountInfoQuery.setAccountPurpose(Constants.ACCOUNT_PURPOSE_3);//清算卡类型
//					}else{
...
//					}
					...
					if(TransCodeConst.WITHDROW.equals(generationType)){
						...
						if(accountInfos == null || 0 == accountInfos.size()){
							logger.info("提现走账户系统获取提现所需的绑卡信息");
...
//					}
					...
					if ("ACCOUNT_INFO".equals(table) && (accountInfos == null || 0 == accountInfos.size())) {
						accountInfoQuery.setAccountPurpose(Constants.ACCOUNT_PURPOSE_1);// 清算卡类型
						accountInfos = accountInfoManager.queryViewByUserIdAndPurpose(accountInfoQuery);
					}
					if ("ACCOUNT_INFO".equals(table) && (accountInfos == null || 0 == accountInfos.size())) {
						accountInfoQuery.setAccountPurpose(Constants.ACCOUNT_PURPOSE_4);// 结算提现一体卡
						accountInfos = accountInfoManager.queryViewByUserIdAndPurpose(accountInfoQuery);
					}
...
//					}
					...
					if ("CORPORAT_ACCOUNT_INFO".equals(table)){
						...
						for(TransOrderInfo transOrderInfo:transOrderInfos){
							if (transOrderInfo.getOrderNo().equals(transDaysSummary.getSummaryOrders())) {
								corQuy.setAccountNumber(transOrderInfo.getInterMerchantCode());//用户Id
								break;
							}
						}
						corQuy.setRootInstCd(transDaysSummary.getRootInstCd());//机构号
						corAccountInfos= corporatAccountInfoManager.queryList(corQuy);
					}
					//准备数据调用接口写入代收付
					GenerationPayment generationPayment=new GenerationPayment();
					generationPayment.setUserId(transDaysSummary.getUserId());
					if("ACCOUNT_INFO".equals(table) && (null == accountInfos || 0 == accountInfos.size())){
					...
					if(Constants.KZ_ID.equals(transDaysSummary.getRootInstCd())){
...
//					}
			...
			RkylinMailUtil.sendMailThread(merchantId + "的 " + generationType +"汇总失败", "失败原因"+e.getMessage(), TransCodeConst.GENERATION_PAY_ERROR_TOEMAIL);
		}
...
/*	    	//代收付结果更新订单系统 20150603 end    	
			//********特殊处理：学生还款给课栈 成功后 ， 发起课栈代付交易给P2P 20150603 start
	    	Map<String,String> rtnMapP2P= this.withholdToP2P(Constants.KZ_ID, TransCodeConst.THIRDPARTYID_KZP2PZZH);
    		if(!"P0".equals(rtnMapP2P.get("errCode"))){
    			rtnMap.put("errCode", rtnMapP2P.get("errCode"));
   			rtnMap.put("errMsg", "发起课栈代付交易给P2P失败：" + rtnMapP2P.get("errMsg"));
    		}
			//********特殊处理：学生还款给课栈 成功后 ， 发起课栈代付交易给P2P 20150603 end
	    	//调用晶晶的存储过程把interest的数据过渡至历史表  20150615 start
            ...
            if(null==paramInterest||!String.valueOf(paramInterest.get("on_err_code")).equals("0")){
    			...
    			rtnMap.put("errMsg", "维护贷款还款表历史数据失败！");
    			logger.error("维护贷款还款表历史数据失败！");
    			return rtnMap;
            }	    	
	    	//调用晶晶的存储过程把interest的数据过渡至历史表20150615 end
			// 调用晶晶的存储过程把成功的代收付过渡至历史表
