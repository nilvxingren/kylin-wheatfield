	...
	//			excelList.add(paraMap);
				...
				for (TransOrderInfo tmpbean:rtnList) {
					...
					if (!TransCodeConst.CHARGE.equals(tmpbean.getFuncCode())) {
						...
						sumAmount = new BigDecimal(tmpbean.getAmount().toString());
						if (null != tmpbean.getProfit() && !"".equals(tmpbean.getProfit())) {
							sumProfit = new BigDecimal(tmpbean.getProfit().toString());
							sumAmount = sumAmount.add(sumProfit);
						}
						paraMap.put("F_5", sumAmount.longValue());//交易金额
...
//				urlKey.add(fileUrl);
			...
			Map paraMap = new HashMap();
			Map configMap = new HashMap();
			paraMap.put("ROOT","'"+ Constants.FN_ID+"','"+Constants.KZ_ID+"'");
			paraMap.put("FUNC_CODE", "'"+ TransCodeConst.CHARGE+"'");
